...
typedef te_bool(*GLIsTextureProc)(te_uint);
...
te_int tea_init(te_config_t *config) {
    ...
    vertex->handle = base()->vbo;
    vertex->data = TEA_MALLOC(vertex->size);
    te_vao_t vao = base()->vao;
    TEA_ASSERT(vertex->data != NULL, "Failed to alloc memory for default vertex data\n");
    tea_bind_vao(vao);
    tea_vao_attach_vertex(vertex);
...
te_void tea_end(te_void) {
    ...
    int size = tea_vertex_offset(vertex);
...
te_void tea_glGenTextures(te_sizei n, te_texture_t *textures) {
    CALL_GL(GenTextures)(n, textures);
}

te_void tea_glDeleteTextures(te_sizei n, const te_texture_t *textures) {
    CALL_GL(DeleteTextures)(n, textures);
}

te_bool tea_glIsTexture(te_texture_t tex) {
    return CALL_GL(IsTexture)(tex);
}

te_void tea_glBindTexture(te_enum target, te_texture_t texture) {
    CALL_GL(BindTexture)(target, texture);
}

te_void tea_glTexImage1D(te_enum target, te_int level, te_int internalFormat, te_sizei width, te_int border, te_enum format, te_enum type, const te_void *data) {
    CALL_GL(TexImage1D)(target, level, internalFormat, width, border, format, type, data);
}

te_void tea_glTexImage2D(te_enum target, te_int level, te_int internalFormat, te_sizei width, te_sizei height, te_int border, te_enum format, te_enum type, const te_void *data) {
    CALL_GL(TexImage2D)(target, level, internalFormat, width, height, border, format, type, data);
}

te_void tea_glTexImage3D(te_enum target, te_int level, te_int internalFormat, te_sizei width, te_sizei height, te_sizei depth, te_int border, te_enum format, te_enum type, const te_void *data) {
    CALL_GL(TexImage3D)(target, level, internalFormat, width, height, depth, border, format, type, data);
}

te_void tea_glTexSubImage1D(te_enum target, te_int level, te_int xoffset, te_sizei width, te_enum format, te_enum type, const te_void *data) {
    CALL_GL(TexSubImage1D)(target, level, xoffset, width, format, type, data);
}

te_void tea_glTexSubImage2D(te_enum target, te_int level, te_int xoffset, te_int yoffset, te_sizei width, te_sizei height, te_enum format, te_enum type, const te_void *data) {
    CALL_GL(TexSubImage2D)(target, level, xoffset, yoffset, width, height, format, type, data);
}

te_void tea_glTexSubImage3D(te_enum target, te_int level, te_int xoffset, te_int yoffset, te_int zoffset, te_sizei width, te_sizei height, te_sizei depth, te_enum format, te_enum type, const te_void *data) {
    CALL_GL(TexSubImage3D)(target, level, xoffset, yoffset, zoffset, width, height, depth, format, type, data);
}

te_void tea_glTexParameteri(te_enum target, te_uint param, te_int value) {
    CALL_GL(TexParameteri)(target, param, value);
}
...
te_void tea_vao_attach_vertex(te_vertex_t *vertex) {
    TEA_ASSERT(vertex != NULL, "Vertex cannot be NULL");
    tea_bind_buffer(TEA_ARRAY_BUFFER, vertex->handle);
...
te_batch_t* tea_batch(te_uint size) {
    TEA_ASSERT(size != 0, "Batch size needs to be > 0");
    te_batch_t *batch = (te_batch_t*)TEA_MALLOC(sizeof *batch);
    TEA_ASSERT(batch != NULL, "Cannot alloc memory for batch");
    memset(batch, 0, sizeof *batch);

    batch->handle = tea_buffer();
    batch->size = size * sizeof(float);

    return batch;    
}

te_void tea_free_batch(te_batch_t *batch) {
    if (!batch)
        return;
    
    if (batch->handle != 0)
        tea_free_buffer(&batch->handle);
    if (batch->data != NULL)
        TEA_FREE(batch->data);
    TEA_FREE(batch);
}

te_void tea_write_batch_data(te_batch_t *batch, te_int size, te_void *data) {
    ...
    if (size > batch->size) {
        ...
        TEA_ASSERT(batch->data != NULL, "Cannot realloc memory for batch data");
    }

    memset(batch->data, 0, batch->size);
    memcpy(batch->data, data, size);
}

te_void tea_write_batch_subdata(te_batch_t *batch, te_int offset, te_int size, te_void *data) {
    TEA_ASSERT(batch != NULL, "Batch cannot be NULL");
    if (!data || size <= 0)
        return;

    te_int nsize = offset + size;
    if (nsize > batch->size) {
        batch->size = nsize;
        batch->data = TEA_REALLOC(batch->data, nsize);
    }

    memcpy(batch->data + offset, data, size);
}

te_void tea_clear_batch(te_batch_t *batch) {
    TEA_ASSERT(batch != NULL, "Batch cannot be NULL");
    memset(batch->data, 0, batch->size);
}

te_void tea_flush_batch(te_batch_t *batch) {
    TEA_ASSERT(batch != NULL, "Batch cannot be NULL");
    tea_bind_buffer(TEA_ARRAY_BUFFER, batch->handle);
    tea_buffer_subdata(TEA_ARRAY_BUFFER, 0, batch->top, batch->data);
    tea_bind_buffer(TEA_ARRAY_BUFFER, 0);
}

te_void tea_draw_batch(te_batch_t *batch, te_enum mode, te_int start, te_int count) {
    TEA_ASSERT(batch != NULL, "Batch cannot be NULL");
    tea_bind_buffer(TEA_ARRAY_BUFFER, batch->handle);
    tea_draw_arrays(mode, start, count);
    tea_bind_buffer(TEA_ARRAY_BUFFER, 0);
}

te_void tea_resize_batch(te_batch_t *batch, te_uint size) {
    TEA_ASSERT(batch != NULL, "Batch cannot be NULL");
    if (size < batch->size) return;
...
te_vertex_format_t* tea_vertex_format(te_void) {
    te_vertex_format_t *format = (te_vertex_format_t*)TEA_MALLOC(sizeof *format);
...
te_void tea_draw_vertex_part(te_vertex_t *vertex, te_enum mode, te_int start, te_int size) {
    TEA_ASSERT(vertex != NULL, "Vertex cannot be NULL");
    tea_draw_arrays(mode, start, size);
}
...
te_int tea_vertex_data_offset(te_vertex_t *vertex) {
    TEA_ASSERT(vertex != NULL, "Vertex cannot be NULL");
    return vertex->offset;
}

te_int tea_vertex_data_size(te_vertex_t *vertex) {
    TEA_ASSERT(vertex != NULL, "Vertex cannot be NULL");
    return vertex->size;
}
...
te_int tea_vertex_count(te_vertex_t *vertex) {
    TEA_ASSERT(vertex != NULL, "Vertex cannot be NULL");
    te_uint size = 0;
    te_vertex_format_t *format = vertex->format;
    if (format->stride != 0)
...
static GLUseProgramProc tea_glUseProgram;
static GLGetUniformLocationProc tea_glGetUniformLocation;
#define TEA_UNIFORM_IMPL_TEST_X(X, T)\
static GLUniform1##X##Proc tea_glUniform1##X;\
static GLUniform2##X##Proc tea_glUniform2##X;\
static GLUniform3##X##Proc tea_glUniform3##X;\
static GLUniform4##X##Proc tea_glUniform4##X;\
static GLUniform1##X##vProc tea_glUniform1##X##v;\
static GLUniform2##X##vProc tea_glUniform2##X##v;\
static GLUniform3##X##vProc tea_glUniform3##X##v;\
static GLUniform4##X##vProc tea_glUniform4##X##v

TEA_UNIFORM_IMPL_TEST_X(f, te_float);
TEA_UNIFORM_IMPL_TEST_X(i, te_int);

static GLUniformMatrix2fvProc tea_glUniformMatrix2fv;
static GLUniformMatrix3fvProc tea_glUniformMatrix3fv;
static GLUniformMatrix4fvProc tea_glUniformMatrix4fv;